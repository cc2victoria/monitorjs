...
import { BrowerClient, BrowserPlugin, ReportEvent, SendEvent } from '@monitorjs/types';
import { getStorageKey, SDK_NAME, SDK_VERSION, COMMON_MONITOR_PLUGIN_NAME } from '../config';
 ...
 * sid: sessionId
 */
export class CommonMonitorPlugin implements BrowserPlugin {
  ...
  public setupOnce = (client: BrowerClient): void => {
    ...
    const getSid = () => {
      ...
      if (sid) {
        ...
        sid =
          getSessionStorageItem(key) || getCookieByName('sid') || getStorageItem('sid') || getSessionStorageItem('sid');

        if (!sid) {
          sid = uuid4();

          setSessionStorageItem(key, sid);
        }
